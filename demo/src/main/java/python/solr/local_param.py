...
class LocalParam(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.python.solr = pysolr.Solr('http://10.10.17.117:8983/python.solr/test1', timeout=10)
        list = []
        ...
        self.python.solr.add(list, commit=True)
        pass

    def testBaseSyntax(self):
        queryStr = "{!df='foo_s'}foo*"
        results = self.python.solr.search(queryStr)
        docs = results.docs
